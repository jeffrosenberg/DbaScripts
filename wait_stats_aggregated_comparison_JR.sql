...
--
-- Added a reference to a second, earlier set of wait stats to be excluded from 
-- the current stats; the results will then be calculated on the difference.
-- Included this reference in the CTEs so I would not have to edit the 
-- final select statement for the sake of consistency.
...
WITH total_waits AS
  ...
  WHERE wait_type NOT IN (
    ...
    SELECT wait_type, wait_s, resource_s, signal_s, wait_count
    FROM DBUtilities.logs.wait_stats_20161201_0745
  ), Waits AS (
    SELECT tot.wait_type,
           tot.WaitS - COALESCE(ex.wait_s, 0.0) AS WaitS,
           tot.ResourceS - COALESCE(ex.resource_s, 0.0) AS ResourceS,
           tot.SignalS - COALESCE(ex.signal_s, 0.0) AS SignalS,
           tot.WaitCount - COALESCE(ex.wait_count, 0.0) AS WaitCount,
		       100.0 * ((tot.WaitS - COALESCE(ex.wait_s, 0.0)) 
                    / SUM (tot.WaitS - COALESCE(ex.wait_s, 0.0)) OVER()) AS Percentage
    FROM total_waits AS tot
    LEFT OUTER JOIN excluded_waits AS ex
      ON ex.wait_type = tot.wait_type
  )
SELECT
