...
SELECT TOP 100
    ...
    (total_logical_reads + (total_logical_writes * 5))*execution_count AS jeffs_pain_weighting,
...
WHERE [dbid] >= 5 AND DB_NAME(dbid) IS NOT NULL
  ...
  AND last_execution_time >= DATEADD(day, DATEDIFF(day, 0, GETDATE()), 0) --TODAY ONLY
--ORDER BY io_weighting DESC;
--ORDER BY jeffs_pain_weighting DESC;
